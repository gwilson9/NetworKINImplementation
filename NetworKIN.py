	...
	with open(filename) as infile:
		for line in infile:
			...
			if (i == 0):
				...
				tokens = line.split('\t')
				try:
					#Will need to split ENSEMBL Isoform here
					id = tokens[0].rstrip()
				except:
...
#	blastDB = os.path.join(datadir, "%s.protein.sequences.fa"%(organism)).replace(' ', '\\ ')
	#Changed
	blastDB = dir + "/MouseProteome_20150814_IDsOnly_Ensembl74NoIsoforms_NoNAs.fasta"
	...
	if True and os.path.isfile(dir + "/blast_out_Ensembl74.txt"):
		sys.stderr.write("Reading Blast Output\n")
		blast_out = ReadLines(dir + "/blast_out_Ensembl74.txt")
	else:
...
def loadSTRINGdata(string2incoming, datadir, number_of_processes):
	...
	while (True):
		...
		if (string2 in string2incoming):
			...
			#sys.stderr.write("string2 %s not in string2incoming\n"%string2)
			pass

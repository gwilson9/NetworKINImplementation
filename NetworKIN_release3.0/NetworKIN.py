...
# Run Netphorest
def runNetPhorest(id_seq, id_pos_res, save_diskspace, number_of_processes, number_of_active_processes = 1, fast = False, leave_intermediates = True):
	if number_of_active_processes < 2:
		raise "Number of maximum threads is less than 2"
	id_pos_tree_pred = {}

	sys.stderr.write("\nrunNetPhorest() Parameters:\nLength id_seq: %s\n"%len(id_seq))
	sys.stderr.write("Length id_pos_res: %s\n"%len(id_pos_res))
	...
	for i in range(number_of_processes):
		sys.stderr.write("Creating temp file #%s"%i)
		file_in[i] = tempfile.NamedTemporaryFile()
		
		if fast or leave_intermediates:
			sys.stderr.write("Fast or Intermiates")
			if save_diskspace:
	...
	sys.stderr.write("Num processes: %s\n"%number_of_processes)	
...
def parseNetphorestFile(filename, id_pos_res, save_diskspace = True):
	...
	# try:
	# 	sys.stderr.write("Trying netphorest_results\n")
	# 	#netphorest_results = myPopen(command)
	# 	netphorest_results = ReadLines("home/gary/NetwoKINtmp/tmplu5cuL_unzip.txt")
	# 	sys.stderr.write("Got results\n")
	# 	#sys.stderr.write("Netphorest_resuts: %s\n"%netphorest_results)
	# except:
	# 	sys.stderr.write("Going to sleep for 1 hour to give you time to debug:\nCrashed with '%s'\n"%command)
	# 	time.sleep(3600)

	#netphorest_results = ReadLines("/home/gary/Documents/tmplu5cuL_unzip.txt")

	uniToEnsDict = {}

	#import pdb; pdb.set_trace()

	with open("/home/gary/Documents/20180925_NetworKIN_Hornberger_RapMIC_Redo/uniprotToEnsembl74.txt") as infile:
		lines = infile.readlines()

		for line in lines:
			lineParts = line.split('\t')
			if len(lineParts) > 1:
				#sys.stderr.write("Length lineParts: %s"%len(lineParts))
				#if(lineParts[0] != "#N/A\r\n"):
				uniToEnsDict[lineParts[0].rstrip()] = lineParts[1].rstrip()
				#sys.stderr.write("ensToUniDict[%s]: %s\n"%(lineParts[1],lineParts[0]))
	...
	with open(filename) as infile:
		for line in infile:
			sys.stderr.write("Temp file line: %s"%j)
			j = j + 1



	i = 0
	sys.stderr.write("Temp File: %s\n"%filename)
	with open(filename) as infile:
		for line in infile:

			sys.stderr.write("Length id_pos_tree_pred: %s\n"%len(id_pos_tree_pred))
			#if(re.match('^#',line)):
			if (i == 0):
				...
				tokens = line.split('\t')
				try:
					if not ("ENSP" in tokens[0]):
						id = uniToEnsDict[tokens[0]].rstrip()
					else:
						id = tokens[0].rstrip()
				except:
					sys.stderr.write("Uniprot ID %s not in uniToEnsDict"%tokens[0])
					continue
				...
				try:
					if tree not in limitTrees:
						sys.stderr.write("Not in limitTrees")
						continue
				except:
					pass
				try:
				...
				if (id in id_pos_res and pos in id_pos_res[id]) or id_pos_res == {}:
					...
					#if not (id in id_pos_res):
					#	sys.stderr.write("ID %s not in id_pos_res\n"%id)						
					#	if not (pos in id_pos_res[id]):
					#		sys.stderr.write("Pos %s not in id_pos_res[%s]\n"%(pos, id))

					#sys.stderr.write("Failed Checks")
					pass
...
def loadSTRINGdata(string2incoming, datadir, number_of_processes):
	...
	# Make Ensemble to Uniprot Dictionary
	"""
	file = open("/home/gary/Documents/20180924_NetworKIN_Hornberger_Human_NoIsoform/uniprotToEnsembl74.txt")
	ensToUniDict = {}
	while(True):
		line = file.readline()
		...
		if len(lineParts) > 1:
			#sys.stderr.write("Length lineParts: %s"%len(lineParts))
			ensToUniDict[lineParts[1]] = lineParts[0]
			#sys.stderr.write("ensToUniDict[%s]: %s\n"%(lineParts[1],lineParts[0]))
			#symbols.stderr.write("Uniprot ID: %s, ENSP:%s\n")%(lineParts[1],lineParts[0])
	"""		
	...
	while (True):
		line = f.readline()
		#sys.stderr.write("STRING Line: %s\n"%line)
		if line == '':
			break
		line = line.strip()
		tokens = line.split('\t')

		#sys.stderr.write("Length tokens: %s\n"%len(tokens))
		...
		"""
		if(string1_ens and ensToUniDict.has_key(string1_ens)):
			#Convert To UniprotID
			#string1 = ensToUniDict[string1_ens]

			#Use Ensembl ID
			string1 = string1_ens

		if(string2_ens and ensToUniDict.has_key(string2_ens)):
			#Convert to UniprotID
			#string2 = ensToUniDict[string2_ens]
		...
		if (string2 in string2incoming):
			#sys.stderr.write("string2 (%s) is in string2incoming.\n"%string2)
	...
	sys.stderr.write("Length tree_pred_string_data: %s\n"%len(tree_pred_string_data))
...
def printResult(id_pos_tree_pred, tree_pred_string_data, incoming2string, string_alias, string_desc, organism, mode, dir_likelihood_conversion_tbl, map_group2domain):
	...
	sys.stdout.write("Length id_pos_tree_pred: %s\n"%len(id_pos_tree_pred))
	sys.stdout.write("Length tree_pred_string_data: %s\n"%len(tree_pred_string_data))
	sys.stdout.write("Length incoming2string: %s\n"%len(incoming2string))
	sys.stdout.write("string_alias: %s\n"%string_alias)
	sys.stdout.write("string_desc: %s\n"%string_desc)
	sys.stdout.write("Mode: %s\n"%mode)
	sys.stdout.write("Length dir_likelihood_conversion_tbl: %s\n"%len(dir_likelihood_conversion_tbl))
	sys.stdout.write("Length map_group2domain: %s\n"%len(map_group2domain))
...
def Main():
	...
	#import pdb; pdb.set_trace()
	sys.stderr.write("Skipped Set Trace\n")
...
#	blastDir = "/home/gary/Vbox_Share/NetworKIN_release3.0/"
	sys.stderr.write("blastDir: %s\n"%blastDir)
	
	sys.stderr.write("organism: %s\n"%organism)
	
	
	sys.stderr.write("fastafile.name: %s\n"%fastafile.name)
	
	sys.stderr.write("id_pos_res: %s\n"%len(id_pos_res))
	
	sys.stderr.write("id_seq: %s\n"%len(id_seq))
	
	sys.stderr.write("Datadir: %s\n"%options.datadir)
	...
	#netphorestTmpFiles = {}
	#netphorestTmpFiles[0] = CDummy("/home/gary/NetwoKINtmp/tmplu5cuL")


	sys.stderr.write("Length netphorestTmpFiles: %s\n"%len(netphorestTmpFiles))
	...
	for i in range(len(netphorestTmpFiles)):
		...
		sys.stderr.write("NetphorestTmpFile_%s: %s\n"%(i, netphorestTmpFiles[i].name))
		sys.stderr.write("Length id_pos_res: %s\n"%len(id_pos_res))
...
if __name__ == '__main__':
	...
	sys.stderr.write("options.tempdir: %s\n"%options.tmpdir)

	#tempfile.tempdir="/home/gary/Documents/NetworKINtmp/"
